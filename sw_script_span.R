...
download_plays <- function(playname){
  if (!file.exists(paste0("csv/", corpusname, "/", playname, ".csv"))) {
    download.file(paste0("https://dracor.org/api/corpora/", corpusname, "/play/", playname, "/networkdata/csv"), paste0("csv/", corpora, "/", playname, ".csv"))
  }
  read.csv(paste0("csv/", corpusname, "/", playname, ".csv"), stringsAsFactors = F)
}

#there is no need to download plays in a folder because there are only 25 items :)
...
plot2 <- ggplot(na.omit(df), aes(x = numOfSpeakers, y = CC_dev, 
  ...
  geom_text(aes(label=ifelse(numOfSpeakers>40,as.character(name),'')),hjust=0.5,vjust=1.5, color="black", size=4)
plot2
